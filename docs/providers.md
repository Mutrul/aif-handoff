...
- Docker images resolve the Codex SDK and its matching CLI during image build using `CODEX_VERSION` (`latest` by default, with exact versions and semver ranges supported). The bundled executable is placed on `PATH` and selected through `CODEX_CLI_PATH`; containers mount persistent `~/.codex` auth state (`codex-auth` volume).
